  ...
  def spec = suite("BadActor")(
    test("user with 5 suspect requests in last 10 seconds gets banned"):
      defer:
        ...
        val results = ZIO.foreach(instants)(instant => BadActor.checkReq(ip, instant, suspect = true)).run

        // 6th request should trigger ban check
        val finalResult = BadActor.checkReq(ip, baseTime.plusSeconds(5), suspect = true).run
    ...
    , test("user that has no suspicious requests gets allowed"):
      defer:
        ...
        val results = ZIO.foreach(instants)(instant => BadActor.checkReq(ip, instant, suspect = false)).run
    ...
    , test("user with suspect requests spread over more than 10 seconds gets allowed"):
      defer:
        ...
        val finalResult = BadActor.checkReq(ip, baseTime.plusSeconds(15), suspect = true).run
